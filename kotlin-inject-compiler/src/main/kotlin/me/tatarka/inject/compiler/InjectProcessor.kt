...
class InjectProcessor(
        ...
        private val generateCompanionExtensions: Boolean
): AstProvider by provider {

    fun process(astClass: AstClass, scopedInjects: Collection<AstClass>): FileSpec {
    ...
    private fun collectTypes(
            ...
            scopedInjects: Collection<AstClass>,
            name: String? = null,
            typesWithScope: MutableMap<AstType, AstType> = mutableMapOf()
    ): Context {
        ...
        for (inject in scopedInjects) {
            scoped[inject.type] = inject
        }

        fun addScope(type: AstType, scope: AstType?) {
            if (scope != null) {
        ...
        astClass.visitInheritanceChain { parentClass ->
            for (method in parentClass.methods) {
                if (method.isProvides()) {
                    ...
                    if (method.annotationOf(IntoMap::class) != null) {
                        // Pair<A, B> -> Map<A, B>
                    ...
                    } else if (method.annotationOf(IntoSet::class) != null) {
                        // A -> Set<A>
    ...
    private fun AstParam.isModule() = annotationOf(Module::class) != null

    private fun AstMethod.isProvides(): Boolean = annotationOf(Provides::class) != null
    ...
    private fun Context.find(key: TypeKey): Result? {
        ...
        val element = key.type
        if (element.annotationOf(Inject::class) != null) {
